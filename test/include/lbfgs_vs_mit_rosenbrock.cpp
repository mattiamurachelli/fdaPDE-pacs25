...
int main() {
    ...
    fdapde::LBFGS<n> solver_our(1000, 1e-6, 1e-2, 6);
    ...
    struct Field {
        // function
        double operator()(Eigen::Matrix<double,n,1> x){
            double sum = 0.0;
            for (int i = 0; i < n - 1; ++i) {
    ...
    std::ofstream file("simulations_rosenbrock_lbfgs.csv");
    ...
    for(int i=0; i < num_simulations; ++i) {
        ...
        solver_our.optimize(Field(), x0, fdapde::BacktrackingLineSearch());
        // Stop timer (OUR)
