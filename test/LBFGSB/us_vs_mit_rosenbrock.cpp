...
int main() {
    ...
    params.step_ = 1.0;
    fdapde::LBFGSB<n> solver_our(params);
    ...
    auto fun_gradient = [n](Eigen::Matrix<double,n,1> x) -> Eigen::Matrix<double,n,1> {
    ...
    std::ofstream file("LBFGSB/simulations_rosenbrock.csv");
    ...
    for(int i=0; i < num_simulations; ++i) {
        ...
        solver_our.optimize(fun, fun_gradient, x0, lb, ub, fdapde::MoreThuenteLineSearch());
        // Stop timer (OUR)
        ...
        if(i==num_simulations-1) {
            std::ofstream traj_("LBFGSB/trajectory_rosenbrock.csv");
            traj_ << "Iter,Point,Fx" << std::endl;
            for(int j=0; j < solver_our.n_iter(); ++j) {
                std::string separator_traj = "";
                traj_ << j+1 << ",";
                for(const auto& value : solver_our.trajectory()[j]) {traj_ << separator_traj << value; separator_traj = ";";}
