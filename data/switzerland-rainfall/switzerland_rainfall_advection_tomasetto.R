...
for(i in 1:rhovec_l){
  ...
  if(i==1) start <- c(pi/2,5) #, 0, 0) # initial value of the parameter x
  if(i!=1) start <- result_optim_LBFGSB_K[i-1,] # for subsequent iteration start from the previous estimated value
  ...
  result_optim_LBFGSB_K[i,] <- optim(par = start, fn = function_to_optimize_K, b=b, method = "L-BFGS-B", 
  ...
  print(paste0("optim:", result_optim_LBFGSB_K[i,1], " ", result_optim_LBFGSB_K[i,2]))
  ## GCV
...
index_K <- which.min(GCV_smooth)

angle <- result_optim_LBFGSB_K[index_K,1]
intensity <- result_optim_LBFGSB_K[index_K,2]

Q <- cbind(c(cos(angle),sin(angle)),c(-sin(angle),cos(angle)))
sigma <- matrix(c(1,0,0,intensity)/sqrt(intensity),nrow=2,ncol=2,byrow=TRUE)
kappa <- Q%*%sigma%*%solve(Q)
#angle_b <- result_optim_LBFGSB[index,3] 
#r_b <- result_optim_LBFGSB[index,4]
#b <- r_b * c(cos(angle_b), sin(angle_b))

# Estimating Advection term
start_time <- Sys.time()
for(i in 1:rhovec_l){
  
  rho <- rhovec[i]
  
  if(i==1) start <- c(0,0) #, 0, 0) # initial value of the parameter x
  if(i!=1) start <- result_optim_LBFGSB_b[i-1,] # for subsequent iteration start from the previous estimated value
  ...
  #                 - the function to optimize (fn)
  #                 - other parameters that regards the optimization procedure (see help("optim"))
  result_optim_LBFGSB_b[i,] <- optim(par = start, fn = function_to_optimize_b, kappa=kappa, method = "L-BFGS-B", 
                                     lower = lower_b, upper = upper_b)$par
  
  # deal with the fact that the angle is "circular", restart the optimization if it ends at a boundary of [0,pi]
  
  print(paste0("optim:", result_optim_LBFGSB_b[i,1], " ", result_optim_LBFGSB_b[i,2]))
  ## GCV
  b_angle <- result_optim_LBFGSB_b[i,1]
  b_r <- result_optim_LBFGSB_b[i,2]
  
  b <- b_r* c(cos(b_angle), sin(b_angle))
...
index_b <- which.min(GCV_smooth)

angle_b <- result_optim_LBFGSB_b[index_b,1] 
r_b <- result_optim_LBFGSB_b[index_b,2]
b <- r_b * c(cos(angle_b), sin(angle_b))
...
kappa
b
PDE_parameters <- list(K= kappa, b = b, c=0)
smoothing_aniso <- smooth.FEM(locations=data_locations, observations=data, 
                              FEMbasis=basisobj, 
                              PDE_parameters = PDE_parameters, 
                              lambda.selection.criterion='newton', 
...
library(Matrix)
#kappa_cpp = as.matrix(readMM("kappa_tomasetto_cpp.mtx")) # to be implemented
...
# 3d plot smoothings
# Tomasetto - Seconda parte (NO) -----------------------------------------------
# result_optim_LBFGSB_K[index_K,]
# kappa_old = Kappa(result_optim_LBFGSB_K[index_K,1], result_optim_LBFGSB_K[index_K,2])
# b_old = result_optim_LBFGSB_b[index_b,2] * c(cos(result_optim_LBFGSB_b[index_b,1]),
#                                              sin(result_optim_LBFGSB_b[index_b,1]))
# 
# kappa_ref = kappa_old
# b_ref = b_old
# max_iter = 10
...
# tol = 1e-5
# iter = 0
# param_K <- result_optim_LBFGSB_K[index_K,]
# param_b <- result_optim_LBFGSB_b[index_b,]
# while(error_K > tol & error_b > tol & iter < max_iter){
#   print(paste0("----- ", iter , " -----"))
#   
#   kappa_old = kappa
#   print("K old:")
...
#   
#   rho <- rhovec[index_K]
#   param_K <- optim(par = param_K, fn = function_to_optimize_K, b=b, method = "L-BFGS-B", 
#                    lower = lower_K, upper = upper_K)$par
#   kappa <- Kappa(param_K[1], param_K[2])
#   rho <- rhovec[index_b]
#   param_b <- optim(par = param_b, fn = function_to_optimize_b, kappa=kappa, method = "L-BFGS-B", 
#                    lower = lower_b, upper = upper_b)$par
#   
#   b <- param_b[2] * c(cos(param_b[1]), sin(param_b[1]))
#   
#   print("K:")
#   print(kappa)
#   print("b:")
#   print(b)
#   
#   error_K <- norm(kappa - kappa_ref, type = "F") / norm(kappa_old, type="F") 
#   error_b <- norm(matrix(b - b_ref, nrow=2,ncol=1), type = "F") / norm(matrix(b_old, nrow=2,ncol=1), type="F") 
#   iter = iter + 1 
# }
# 
# PDE_parameters <- list(K = kappa, b = b, c = c)
