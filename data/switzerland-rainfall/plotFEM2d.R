plotFEM2d=function(SolutionObj, Nx = 100, Ny = 100, levels=NULL,
                   xlim = NA, ylim = NA, zlim = NULL,out=NULL, ...)
{
  if(is.na(xlim[1]))
  {
    xmin = min(SolutionObj$fit.FEM$FEMbasis$mesh$nodes[,1])
    xmax = max(SolutionObj$fit.FEM$FEMbasis$mesh$nodes[,1])
  }
  else
  {
    xmin = xlim[1]
  ...
  if(is.na(ylim[1]))
  {
    ymin = min(SolutionObj$fit.FEM$FEMbasis$mesh$nodes[,2])
    ymax = max(SolutionObj$fit.FEM$FEMbasis$mesh$nodes[,2])
  }
  else
  {
    ymin = ylim[1]
  ...
  eval_sol_in = eval.FEM(SolutionObj$fit.FEM, locations = eval_points)
  if(!is.null(out)){
